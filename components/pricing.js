...
const TIERS = [
  ...
  { name: "Team", price: "$499", per: "/mo", cta: "Get a demo", featured: true, feats: ["Unlimited connectors", "10k workflows / mo", "Review queue & audit log", "Priority support"] },
  { name: "Enterprise", price: "Custom", per: "", cta: "Talk to us", featured: false, feats: ["SSO & SCIM", "Dedicated infrastructure", "SLA & onboarding", "Solutions engineer"] },
];
  ...
  connectedCallback() {
    ...
    this.innerHTML = TIERS.map(
      (t) => `<div class="price-card${t.featured ? " is-featured" : ""}">
        ...
        <button class="btn ${t.featured ? "btn-primary" : "btn-outline"}" type="button">${t.cta}</button>
      </div>`
    ).join("");
  }
}
